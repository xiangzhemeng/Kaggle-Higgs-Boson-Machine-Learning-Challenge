...
def cross_column_features(x, num_features=0):
    ...
    '''
    # 3s combination of columns
    num_double_features = len(double_features)
    triple_features = list()
    for i in range(num_features):
        # print("Feature(%d/%d)" % (i, num_features-1)) #DEBUG
        for j in range(double_features.shape[1]):
            tmp = x[:, i] * double_features[:, j]
            triple_features.append(tmp)

    triple_features = np.array(triple_features).T

    return np.c_[x, double_features, triple_features]
    '''
